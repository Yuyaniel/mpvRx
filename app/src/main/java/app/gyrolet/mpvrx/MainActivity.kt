...
private fun screenNavTransition(
  ...
  // `speed` is a duration multiplier (lower = faster, higher = slower), matching the
  // rest of the app (see animationSpeed preference). Scaling stiffness by 1/speed makes
  // the animation spring faster/slower accordingly. 1.0x keeps the default spring speed.
  // `T` is inferred from the target animation (Float for fade/scale, IntOffset for slide).
  fun <T> scaled(base: SpringSpec<Float>): SpringSpec<T> =
    spring(dampingRatio = base.dampingRatio, stiffness = base.stiffness / speed)
  ...
  return when (style) {
    ...
    NavigationAnimStyle.Minimal ->
      fadeIn(
        scaled(AppMotion.Spatial.Standard),
      ) togetherWith
        fadeOut(scaled(AppMotion.Spatial.Standard))

    NavigationAnimStyle.FlipFade ->
      (
        scaleIn(
          scaled(AppMotion.Spatial.Expressive),
          initialScale = 0.94f,
        ) +
          fadeIn(
            scaled(AppMotion.Spatial.Expressive),
        ...
        (
          scaleOut(scaled(AppMotion.Spatial.Standard), targetScale = 1.06f) +
            fadeOut(scaled(AppMotion.Spatial.Standard))
        )
      ...
      (
        ...
        (
          slideOutHorizontally(
            ...
            scaleOut(scaled(AppMotion.Spatial.Standard), targetScale = 0.92f) +
            fadeOut(scaled(AppMotion.Spatial.Standard))
        )
      ...
      (
        slideInHorizontally(
          spring(
            ...
            stiffness = AppMotion.Spatial.Snappy.stiffness / speed,
          ),
        ) { it * dir } + fadeIn(scaled(AppMotion.Spatial.Snappy))
      ) togetherWith
        (
          slideOutHorizontally(scaled(AppMotion.Spatial.Standard)) { (-it / 3 * dir) } +
            fadeOut(scaled(AppMotion.Spatial.Standard))
        )
    ...
    NavigationAnimStyle.Default ->
      if (forward) {
        ...
        // Back: the current screen always slides out to the RIGHT; the new screen
        // slides in from the LEFT. Unified so back always recedes toward the right.
        slideInHorizontally(
          scaled(AppMotion.Spatial.Expressive),
        ) { -it / 5 } togetherWith
          slideOutHorizontally(
