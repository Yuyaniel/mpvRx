 ...
 *
 * Changes:
 * - Adds webdav_folder_bookmarks table for the WebDAV folder bookmark feature.
 *   The table has NO foreign key to network_connections on purpose: deleting a
 *   WebDAV storage source must keep its bookmarks.
 */
val MIGRATION_16_17 =
  object : Migration(16, 17) {
    override fun migrate(db: SupportSQLiteDatabase) {
      db.execSQL(
        """
        CREATE TABLE IF NOT EXISTS `webdav_folder_bookmarks` (
          `id` INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
      ...
      db.execSQL(
        "CREATE UNIQUE INDEX IF NOT EXISTS `index_webdav_folder_bookmarks_connectionId_folderPath` " +
          "ON `webdav_folder_bookmarks` (`connectionId`, `folderPath`)",
      )
    }
  }
...
val DatabaseModule =
  module {
    ...
    single {
      get<MpvRxDatabase>().webDavFolderBookmarkDao()
    }

    single {
      app.gyrolet.mpvrx.database.repository.WebDavFolderBookmarkRepository(
        dao = get<MpvRxDatabase>().webDavFolderBookmarkDao(),
      )
    }
