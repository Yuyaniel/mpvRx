...
class NetworkBrowserViewModel(
  ...
  private val webDavBookmarkRepository: WebDavFolderBookmarkRepository by inject()
  ...
  val isCurrentFolderBookmarked: StateFlow<Boolean> =
    webDavBookmarkRepository
      .observeIsBookmarked(connectionId, currentPath)
      .stateIn(
  ...
  /**
   * Bookmark or un-bookmark the currently browsed WebDAV folder.
   */
  fun toggleCurrentFolderBookmark() {
    viewModelScope.launch {
      val connection =
        _connection.value ?: repository.getConnectionById(connectionId) ?: return@launch
      if (isCurrentFolderBookmarked.value) {
        webDavBookmarkRepository.remove(connectionId, currentPath)
      } else {
        webDavBookmarkRepository.add(
          WebDavFolderBookmarkEntity(
            connectionId = connectionId,
            connectionName = connection.name,
            folderPath = currentPath,
            folderName = bookmarkFolderDisplayName(currentPath),
          ),
        )
      }
    }
...
internal fun bookmarkFolderDisplayName(path: String): String {
