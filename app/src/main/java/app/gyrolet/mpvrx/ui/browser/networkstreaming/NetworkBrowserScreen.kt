      ...
      topBar = {
        if (isSearching) {
          ...
          BrowserTopBar(
            ...
            onSearchLeadingActions = {
              WebDavFolderBookmarkAction(
                connection = connection,
                currentPath = currentPath,
                isBookmarked = isCurrentFolderBookmarked,
                onToggle = { viewModel.toggleCurrentFolderBookmark() },
              )
            },
...
/**
 * Top-bar star action that bookmarks / un-bookmarks the currently browsed WebDAV
 * sub-folder. Only shown for WebDAV connections below the storage root.
 */
@Composable
private fun WebDavFolderBookmarkAction(
  ...
  val isBookmarkable =
    connection?.protocol == NetworkProtocol.WEBDAV && currentPath.trimEnd('/').isNotEmpty()
  ...
  IconButton(
    ...
    Icon(
      imageVector = if (isBookmarked) Icons.RoundedFilled.Star else Icons.RoundedFilled.StarBorder,
      contentDescription =
        stringResource(
          if (isBookmarked) R.string.ui_remove_webdav_bookmark else R.string.ui_bookmark_webdav_folder,
        ),
      tint =
        if (isBookmarked) {
