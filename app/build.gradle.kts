...
// Mirrors splits.abi.isUniversalApk: a universal APK is only produced when several
// ABIs are active. With a single targetAbi (e.g. -PtargetAbi=arm64-v8a) there is no
// universal output, so per-ABI outputs must stay enabled even for universal-only
// flavors such as "fongmi".
val hasUniversalApk = activeAbis.size > 1
...
androidComponents {
  ...
  onVariants { variant ->
    ...
    variant.outputs.forEach { output ->
      ...
      if (isUniversalOnly && abi != null && hasUniversalApk) {
